...
{
    public partial class EditProfileAccount : System.Web.UI.Page
    {
        public string txtGT;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            KhachHang khachHang = dbcontext.KhachHangs.FirstOrDefault(s => s.MaKH == id || s.Email == email);
                ...
                var user = dbcontext.TKs.FirstOrDefault(s => s.Email == email);
                ...
                if (user.Quyen != null)
                {
                    lbQuyenName.Text = user.Quyen;
                    //imgUser.ImageUrl = khach.AvatarUser;
                }
                else
                {
                    lbQuyenName.Text = "None";
        ...
        protected void btAdmin_Click(object sender, EventArgs e)
        {
            ...
            var deparment = dbcontext.TKs.SingleOrDefault(p => p.Email == email); //lọc trong xem có email nào == lbDeptid đang được hiện (TK)
            KhachHang client = dbcontext.KhachHangs.SingleOrDefault(p => p.Email == email); //lọc trong xem có email nào == lbDeptid đang được hiện (KH)
            try
            {
                if (client.Email == deparment.Email || client.Email == email || deparment.Email == email)
                {
                ...
                dbcontext.SaveChanges();
                Response.Write("<div class=\"alert alert-success\" role=\"alert\">Cấp quyền cho tài khoản thành công.</div>");
                Response.Write("<script type=\"text/javascript\">");
                Response.Write("setTimeout(function() {");
                Response.Write("var elements = document.getElementsByClassName('alert');");
                Response.Write("for (var i = 0; i < elements.length; i++) {");
        ...
        protected void btManager_Click(object sender, EventArgs e)
        {
            ...
            TK deparment = dbcontext.TKs.SingleOrDefault(p => p.Email == email); //lọc trong xem có email nào == lbDeptid đang được hiện (TK)
            KhachHang client = dbcontext.KhachHangs.SingleOrDefault(p => p.Email == email); //lọc trong xem có email nào == lbDeptid đang được hiện (KH)
            try
            {
                if (client.Email == deparment.Email || client.Email == email || deparment.Email == email)
